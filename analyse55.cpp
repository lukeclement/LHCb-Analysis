...
void MyAnalysis::Execute()
{
    ...
    if(Identity(H1_ProbK, H1_ProbPi, H1_Charge,false,false,false)==1){
      if(H1_Charge==1){
	BPlus++;h_M1->Fill(pow(pow(E1+E2+E3,2)-pow(PX,2)-pow(PY,2)-pow(PZ,2),0.5));
      }else{
	BMinus++;
    ...
    if(pow(pow(E1+E2+E3,2)-pow(PX,2)-pow(PY,2)-pow(PZ,2),0.5)>=5338){
      //std::cout<<BMinus<<";"<<BPlus<<std::endl;
    }
